...
std::vector <std::vector <long double>> r_earth;
std::vector <std::vector <long double>> r_2023BU;
std::vector <std::vector <long double>> r_moon;
std::vector <std::vector <long double>> r_sun;
...
void print_trajectory(long double t, int size, long double *x, long double *mass) {
    ...
    r_earth.push_back(tmp_vector);
    r_2023BU.push_back(tmp_vector);
    r_sun.push_back(tmp_vector);
    r_moon.push_back(tmp_vector);
    int tail = r_earth.size() - 1;
    for(int i = 0; i < 3; i++) {
        r_earth[tail].push_back(x[i]);
        r_2023BU[tail].push_back(x[3 + i]);
        r_sun[tail].push_back(x[6 + i]);
        r_moon[tail].push_back(x[9 + i]);
    }
	ofs << std::endl;
}

void get_arrays_info(std::ofstream & test_ofs) {
    for(size_t i = 0; i < r_moon.size(); i++) {
